...
if (figma.editorType === "dev" && figma.mode === "codegen") {
  ...
  figma.codegen.on("generate", codegenHandle);
} else {
  ...
  io.on("get_locale_data", async () => {
    io.send("get_locale_data", { localeData: getLocaleDataWithCreatingMain() });
  });
  io.on(
    "save_locale_data",
    async (msg) => await saveLocaleData(msg.localeData)
  );
  io.on(
    "print_code_block",
    async (msg) =>
