...
function KeyCombobox({
    ...
    ...[
      {
        ...
        onSelect: () => {
          dispatch(setCurrentDialog({opened: true, type: 'NEW', onDone: (localeItem: LocaleItem) => {
  ...
  const updateText = (_text: LocaleText | LocaleText[], localeItemOrKey: LocaleItem | string) => {
    ...
    if(isArray(_text)) {
      ...
      dispatch(updateTextsInLocaleSelection(_text.map(text => ({
        ...text,
        key: localeItem.key,
  ...
  const updateSelectionOrText = (localeItemOrKey: string | LocaleItem) => {
    console.log('test');
    console.log('updateText in KeyCombobox', localeItemOrKey);
    // dang selection
