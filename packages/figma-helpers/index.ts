...
export const getVariantInSet = (
  ...
  const component = <ComponentNode | null>componentSet.findChild((node) => {
    return (
      node.type == "COMPONENT" &&
      isEqual(node.variantProperties, {
        ...componentSet.defaultVariant.variantProperties,
        ...variantProperties,
      })
    );
  });
...
export const swapVariant = (
  ...
  changedVariantProperties: { [key: string]: string }
) => {
  if (instance.mainComponent.parent) {
    const swapedComponent = instance.mainComponent.parent.findChild((node) => {
      return (
        node.type == "COMPONENT" &&
        isEqual(node.variantProperties, {
          ...instance.mainComponent.variantProperties,
          ...changedVariantProperties,
        })
      );
    });
    if (!swapedComponent) {
...
export const isVariant = (
  ...
  return (
    ...
    node.mainComponent.parent.name == componentName &&
    isMatch(node.mainComponent.variantProperties, variantProperties)
  );
};
...
export const isInstance = (
  ...
  if (componentName) {
    return (
      node &&
      node.type == "INSTANCE" &&
      ((node.mainComponent && node.mainComponent.name == componentName) ||
        (node.mainComponent.parent &&
          node.mainComponent.parent.name == componentName))
    );
  } else {
...
export const findContainer = (
  ...
  return <ContainerNode | null>(
    parentContainer.findOne(
      (node) => isContainer(node) && node.name == containerName
    )
  );
};
